...
class RecordingProvider:
    ...
    async def generate_structured(self, **kwargs):
        ...
        return TaskDeliverable(
            summary="Prepared a plan", deliverable="Milestone 1: validate demand.",
            evidence=["Includes a measurable milestone"], limitations=["Draft only"],
        )


def task(task_id, dependencies=None):
    return TaskRead(
        id=task_id, workflow_id="wf-test", title="Prepare a plan",
        description="Prepare an operating plan.", priority="HIGH", difficulty=2,
        required_role="Business Analyst", minimum_experience_years=2,
        required_skills=["analysis"], expected_output="Operating plan",
        acceptance_criteria=["Include milestones"], estimated_cost=250,
        dependency_task_ids=dependencies or [], revision_count=1,
...
def test_ai_workers_preserve_authoritative_fields_and_pass_dependency_outputs():
    ...
    results = asyncio.run(WorkerAgent().execute_all(
        [task("task-002", ["task-001"]), task("task-001")], ai_service=provider,
    ))
...
def test_worker_dependency_error_is_visible():
    ...
    with pytest.raises(ValueError, match="dependencies"):
        asyncio.run(WorkerAgent().execute_all(
            [task("task-001", ["missing"])], ai_service=RecordingProvider(),
        ))
