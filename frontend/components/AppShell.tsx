...
export function AppShell({ children, mode }: { children: ReactNode; mode?: "demo" | "real" }) {
  ...
  const activeFor = (label: string) => label === "Command Center" ? pathname === "/" : label === "AI Workforce" ? pathname.startsWith("/workers") : label === "Workflow" ? pathname.startsWith("/workflows") : label === "Reports" ? pathname.startsWith("/reports") : label === "Marketing" ? pathname.startsWith("/marketing") : label === "Watcher" ? pathname.startsWith("/watcher") : pathname.startsWith("/executive-summary");
