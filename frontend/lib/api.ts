...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) throw new Error(body?.detail?.message ?? body?.error?.message ?? "The service could not complete the request.");
  return (body as ApiResponse<T>).data;
}
export function createWorkflow(payload: WorkflowCreate) { return request<Workflow>("/workflows", { method: "POST", body: JSON.stringify(payload) }); }
